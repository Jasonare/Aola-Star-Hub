...
in_xlsx = ROOT / 'yabi_info_regenerated_from_100bt_skill_retry_v3_gl.xlsx'
ref_xlsx = ROOT / '原系亚比_100bt_编号_技能_种族值.xlsx'
out_xlsx = ROOT / 'yabi_info_regenerated_from_100bt_skill_retry_v3_gl_processed.xlsx'
...
# 2) map 亚比ID -> dexId[] from ref xlsx (sheet2+sheet3 by 编号集合+页面标题)
...
for ref_df in (ref_s2, ref_s3):
    # by legacy structure: 页面标题 col index 1, 编号集合 col index 3
    for _, r in ref_df.iterrows():
        ...
        matched = [name_to_dex[nm] for nm in names if nm in name_to_dex]  # exact match only
        if not matched:
            continue
        for yid in id_set:
...
# 3) load target workbook (2 sheets)
...
print('rows=', len(s0), len(s1))
print('dex_mapped_rows=', int((s0['dexId'].astype(str) != '').sum()), int((s1['dexId'].astype(str) != '').sum()))
