...
const stageRe=/(提升|提高|上升|增加|增强|降低|下降|削弱|减少).{0,30}(等级|全属性|全能力|双攻|双防|攻防|攻击|防御|特攻|特防|速度|命中|闪避|回避|暴击|普攻|物攻|魔攻)/;
const rows=[];
...
const seen=new Set();
const out=[];
for(const r of rows){const k=r.skillKey||`${r.name}::${r.desc}`; if(seen.has(k)) continue; seen.add(k); out.push(r);}
out.sort((a,b)=>String(a.name).localeCompare(String(b.name),'zh-CN'));
fs.writeFileSync('__conditional_stage_scan.json',JSON.stringify({count:out.length,items:out},null,2),'utf8');
console.log('count='+out.length);
console.log(out.slice(0,40).map(x=>`${x.name} | ${x.desc}`).join('\n'));
