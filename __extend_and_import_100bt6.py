...
ROOT=Path('.')
xlsx=ROOT/'yabi_info_regenerated_from_100bt_6_processed.xlsx'
species_js=ROOT/'aola-species-data.js'
name_json=ROOT/'亚比大全.json'
report=ROOT/'__import_100bt6_extend_report.json'
...
rep={'mode':'extend_and_overwrite_100bt6','source_xlsx':xlsx.name,'skills_dex_keys':len(skills_by_dex),'race_dex_keys':len(races_by_dex),'added_new_dex':len(added),'added_sample':added[:80],'updated_skills':len(set(upd_sk)),'updated_races':len(set(upd_r))}
