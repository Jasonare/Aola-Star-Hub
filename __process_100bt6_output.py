...
ROOT = Path('.')
in_xlsx = ROOT / 'yabi_info_regenerated_from_100bt_6.xlsx'
ref_xlsx = next(x for x in ROOT.glob('*编号_技能_种族值(6).xlsx') if not x.name.startswith('~$'))
out_xlsx = ROOT / 'yabi_info_regenerated_from_100bt_6_processed.xlsx'
...
# 2) build 亚比ID -> dexId list using strict exact match from title tokens
...
for _, r in ref_skill.iterrows():
    ...
    matched = [name_to_dex[nm] for nm in title_names if nm in name_to_dex]  # exact only
    if not matched:
        continue
...
for i, row in s1.iterrows():
    ...
    if pp_raw is None:
        # treat as normal-missing: keep PP as 0, accuracy random in normal set
        s1.at[i, 'PP值'] = 0
